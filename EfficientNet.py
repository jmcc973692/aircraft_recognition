...
def save_checkpoint(state, scheduler_warmup, scheduler_cosine, filename="checkpoint.pth.tar"):
    """
...
def load_checkpoint(model, optimizer, scheduler_warmup, scheduler_cosine, checkpoint_path):
    """
...
def main():
    ...
    basic_transform = CroppedImageTransformer(resize_dims=(380, 380))
    transform_augment = CroppedImageTransformer(resize_dims=(380, 380), augment=True)
    ...
    valid_loader = DataLoader(
        ...
        num_workers=8,  # Adjust according to your machine's specifications
        pin_memory=True,  # Recommended if you are using a GPU for training
    )
    ...
    base_params = [p for p in model.parameters() if p not in bn_params]
    ...
    model_path = f"models/classification_model_EffNetB4_{timestamp}.pth"

    # Check if a checkpoint exists and Load it
    checkpoint_path = "temp/checkpoints/EffNetB4_checkpoint.pt"
    start_epoch, best_valid_loss, no_improvement_count = load_checkpoint(
        model, optimizer, scheduler_warmup, scheduler_cosine, checkpoint_path
    )
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        if epoch < 8:
            ...
            current_lr = scheduler_cosine.get_last_lr()
    ...
    test_transformer = CroppedImageTransformer(resize_dims=(380, 380))  # No augmentation for testing
    test_dataset = CroppedDataset(images=images_test, labels=labels_test, transformer=test_transformer)
    test_loader = DataLoader(test_dataset, batch_size=10, shuffle=False, num_workers=8, pin_memory=True)
